...
> ⚠️ **坐标约定提醒（容易踩坑）**：OPV2V 的 GT LiDAR `.pcd` 是 CARLA/UE 约定（Z-up），而 MapAnything 推理输出点云默认是 OpenCV 约定（Y-down）。  
> 生成 HTML 报告/计算 Chamfer、BEV IoU 前必须把预测点云从 OpenCV 转回 CARLA；本仓库的 `scripts/batch_eval.py`（点云指标）与 `scripts/make_batch_eval_pcd_html.py`（HTML 可视化）已做此转换，旧版本产物若出现“点云被压扁/竖直拉伸”，直接重新生成 HTML 即可。
