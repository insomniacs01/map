...
def _patch_torch_hub_load_for_offline_dinov2() -> None:
    """Prefer a locally cached DINOv2 repo when available.

    `uniception`'s DINOv2 encoder loads models via `torch.hub.load("facebookresearch/dinov2", ...)`,
    which may try to reach GitHub even when a cache exists. In network-restricted environments we
    rewrite that call to point at the cached repo directory (if present).
    """
    ...
    if getattr(torch.hub, "_mapanything_offline_dinov2_patch", False):
        return
    ...
    def _patched_load(repo_or_dir, model, *args, **kwargs):
        if repo_or_dir == "facebookresearch/dinov2":
            ...
            patched_kwargs["source"] = "local"
            return original_load(str(cached_repo), model, *args, **patched_kwargs)
        return original_load(repo_or_dir, model, *args, **kwargs)

    torch.hub.load = _patched_load
    torch.hub._mapanything_offline_dinov2_patch = True
