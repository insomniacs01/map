...
def load_images(
    ...
    # NOTE: VGGT expects `data_norm_type=identity` (no mean/std normalization).
    if norm_type in IMAGE_NORMALIZATION_DICT.keys():
        img_norm = IMAGE_NORMALIZATION_DICT[norm_type]
        ImgNorm = tvf.Compose(
        ...
        ImgNorm = tvf.ToTensor()
    else:
        raise ValueError(
            f"Unknown image normalization type: {norm_type}. Available options: "
            f"identity or {list(IMAGE_NORMALIZATION_DICT.keys())}"
        )
