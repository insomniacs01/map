...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None, max_iter=None):
        ...
        if len_iterable > 0:
            ...
            print("{} Total time: {} (0 it)".format(header, total_time_str))
...
class NativeScalerWithGradNormCount:
    ...
    def __call__(
        ...
        if update_grad:
            ...
            # Extra safety: a finite forward loss can still yield non-finite gradients
            # (e.g., acos near 1.0, division-by-zero, etc.). If we step the optimizer
            # with such gradients, model weights can become NaN/inf and permanently
            # poison the run. GradScaler's internal inf check runs during `unscale_`,
            # but subsequent operations (like grad clipping) can still introduce NaNs.
            ...
            for group in optimizer.param_groups:
                ...
                if non_finite_grad:
                    break
            if non_finite_grad:
                print("[WARN] Non-finite gradients detected; skipping optimizer step.", force=True)
                optimizer.zero_grad(set_to_none=True)
                # Backoff the scaler if possible (keeps behavior robust across torch versions).
                try:
                    self._scaler.update(self._scaler.get_scale() * 0.5)
                except TypeError:
                    self._scaler.update()
                return norm
...
def load_model(train_args, model_without_ddp, optimizer, loss_scaler):
    ...
    """
    def _find_non_finite_param(state_dict):
    ...
    if train_args.resume and train_args.resume_ckpt is not None:
        ...
        bad_param = _find_non_finite_param(checkpoint.get("model", {}))
        if bad_param is not None:
            print(
                "Resume checkpoint has non-finite values ({}). Skipping resume.".format(
                    bad_param
                )
            )
            train_args.resume = False
            train_args.resume_ckpt = None
            return best_so_far
...
def get_parameter_groups(
    ...
    # `parameter_group_vars` is what we actually feed to the optimizer.
    # Avoid dumping/serializing full parameter name lists (can be huge and stall
    # multi-GPU startup when stdout is tee'd to disk).
    parameter_group_names = {}
    parameter_group_vars = {}
    parameter_group_name_to_idx_map = {}
    parameter_group_idx_to_name_map = {}
    mapping_index = 0
    max_name_examples_per_group = 5
    ...
    for name, param in model.named_parameters():
        ...
        parameter_group_names[group_name]["num_params"] += 1
        parameter_group_names[group_name]["numel"] += int(param.numel())
        if len(parameter_group_names[group_name]["examples"]) < max_name_examples_per_group:
            parameter_group_names[group_name]["examples"].append(name)
