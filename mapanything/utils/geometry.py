...
def quaternion_to_rotation_matrix(quat):
    ...
    # Ensure the quaternion is normalized (be robust to near-zero norms / NaNs).
    # Some finetuning regimes can temporarily push quaternions close to 0, and the
    # naive normalization would yield NaNs that then poison pointmap/pose tensors.
    quat = torch.nan_to_num(quat, nan=0.0, posinf=0.0, neginf=0.0)
    eps = 1e-8
    norms = quat.norm(dim=1, keepdim=True)
    quat = quat / norms.clamp(min=eps)
    if torch.any(norms < eps):
        ident = torch.tensor(
            [0.0, 0.0, 0.0, 1.0], device=quat.device, dtype=quat.dtype
        ).unsqueeze(0)
        quat = torch.where(norms < eps, ident, quat)
...
def quaternion_inverse(quat):
    ...
    # NOTE: Quaternion inversion is numerically fragile when quats approach 0-norm
    # during finetuning (common early in training or after an unstable update).
    # A naive inverse would create inf/nan values that can poison relative-pose
    # computations and downstream losses.
    quat = torch.nan_to_num(quat, nan=0.0, posinf=0.0, neginf=0.0)
    eps = 1e-8
    norms = quat.norm(dim=1, keepdim=True)
    quat_unit = quat / norms.clamp(min=eps)
    if torch.any(norms < eps):
        ident = torch.tensor(
            [0.0, 0.0, 0.0, 1.0], device=quat.device, dtype=quat.dtype
        ).unsqueeze(0)
        quat_unit = torch.where(norms < eps, ident, quat_unit)

    # For unit quaternions, inverse == conjugate.
    inv_quat = quat_unit.clone()
...
def quaternion_multiply(q1, q2):
    ...
    # Sanitize and normalize to unit quaternions (stable composition).
    q1 = torch.nan_to_num(q1, nan=0.0, posinf=0.0, neginf=0.0)
    q2 = torch.nan_to_num(q2, nan=0.0, posinf=0.0, neginf=0.0)
    eps = 1e-8
    n1 = q1.norm(dim=1, keepdim=True)
    n2 = q2.norm(dim=1, keepdim=True)
    q1 = q1 / n1.clamp(min=eps)
    q2 = q2 / n2.clamp(min=eps)
    if torch.any(n1 < eps) or torch.any(n2 < eps):
        ident = torch.tensor(
            [0.0, 0.0, 0.0, 1.0], device=q1.device, dtype=q1.dtype
        ).unsqueeze(0)
        q1 = torch.where(n1 < eps, ident, q1)
        q2 = torch.where(n2 < eps, ident, q2)
    ...
    # Stack the components and re-normalize to prevent drift / NaNs.
    qm = torch.stack([x, y, z, w], dim=1)
    qm = torch.nan_to_num(qm, nan=0.0, posinf=0.0, neginf=0.0)
    nm = qm.norm(dim=1, keepdim=True)
    qm = qm / nm.clamp(min=eps)
    if torch.any(nm < eps):
        ident = torch.tensor(
            [0.0, 0.0, 0.0, 1.0], device=qm.device, dtype=qm.dtype
        ).unsqueeze(0)
        qm = torch.where(nm < eps, ident, qm)
...
def convert_ray_dirs_depth_along_ray_pose_trans_quats_to_pointmap(
    ...
    # Normalize the quaternions to ensure they are unit quaternions.
    # Avoid division-by-zero which would introduce NaNs and poison pointmaps/losses.
    pose_quats = torch.nan_to_num(pose_quats, nan=0.0, posinf=0.0, neginf=0.0)
    eps = 1e-8
    q_norm = torch.norm(pose_quats, dim=-1, keepdim=True)
    pose_quats = pose_quats / q_norm.clamp(min=eps)
    if torch.any(q_norm < eps):
        ident = torch.tensor(
            [0.0, 0.0, 0.0, 1.0], device=pose_quats.device, dtype=pose_quats.dtype
        )
        pose_quats = torch.where(q_norm < eps, ident, pose_quats)
