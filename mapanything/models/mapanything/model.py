...
class MapAnything(nn.Module, PyTorchModelHubMixin):
    ...
    def _initialize_info_sharing(self, info_sharing_config):
        ...
        # HuggingFace config.json cannot serialize python callables. Older exported
        # configs sometimes store placeholders like `"mlp_layer": "dummy"` which
        # should fall back to the default implementation in UniCeption.
        mlp_layer = info_sharing_config["module_args"].get("mlp_layer")
        if isinstance(mlp_layer, str):
            if mlp_layer.lower() in {"dummy", "default", "none"}:
                info_sharing_config["module_args"].pop("mlp_layer", None)
            else:
                raise ValueError(
                    f"Unsupported string mlp_layer='{mlp_layer}'. "
                    "Provide a callable, or use 'dummy' to select the default."
                )
    ...
    def _encode_n_views(self, views):
        ...
        """
        num_views = len(views)
        cache_flags = ["encoder_features" in view for view in views]
        if all(cache_flags):
            return [view["encoder_features"] for view in views]
        if any(cache_flags):
            raise ValueError(
                "Mixed cached and uncached encoder features in the same batch."
            )
        ...
        # If the encoder is frozen (common in finetune/debug workflows), avoid tracking
        # gradients through it to drastically reduce activation memory.
    ...
    def downstream_head(
        ...
        scale_final_output = torch.nan_to_num(
            scale_final_output, nan=1.0, posinf=1.0e4, neginf=1.0e-4
        )
        # Avoid extremely tiny scales that can amplify division in losses and trigger NaNs.
        scale_final_output = scale_final_output.clamp(min=1.0e-4, max=1.0e4)
