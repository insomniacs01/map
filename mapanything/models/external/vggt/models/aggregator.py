...
class Aggregator(nn.Module):
    ...
    def __init__(
        ...
        self.use_causal_global = bool(use_causal_global)
    ...
    def wrap_module_with_gradient_checkpointing(self, module: nn.Module):
        ...
        class _CheckpointingWrapper(module.__class__):
            ...
            def forward(self, *args, **kwargs):
                # PyTorch >=2.0 recommends `use_reentrant=False` for better compatibility
                # with torch.compile/SDPA kernels and to avoid upcoming deprecation.
                try:
                    return checkpoint(super().forward, *args, use_reentrant=False, **kwargs)
                except TypeError:
                    # Fallback for older PyTorch versions that do not support `use_reentrant`.
    ...
    def forward(
    ...
    ) -> Union[Tuple[List[torch.Tensor], int], Tuple[List[torch.Tensor], int, list]]:
        """
        Args:
        ...
        if use_cache and past_key_values and past_key_values[0] is not None:
        ...
        if use_cache and S > 1:
            logger.warning("Use KV cache expects S=1, got S=%s", S)
        ...
        if use_cache:
            token_idx = 0 if S_true == 1 else 1
            camera_token = (
                self.camera_token[:, token_idx : token_idx + 1, ...]
                .expand(B, 1, *self.camera_token.shape[2:])
                .reshape(B * S, *self.camera_token.shape[2:])
            )
            register_token = (
                self.register_token[:, token_idx : token_idx + 1, ...]
                .expand(B, 1, *self.register_token.shape[2:])
                .reshape(B * S, *self.register_token.shape[2:])
            )
        else:
                    ...
                    if use_cache:
                        tokens, global_idx, global_intermediates, new_kv = (
                            self._process_global_attention(
                                ...
                                global_idx,
                                pos=pos,
                                past_key_values_block=(
                                    past_key_values[global_idx]
                                    if past_key_values
                                    and past_key_values[global_idx] is not None
                                    else None
                                ),
                                use_cache=True,
    ...
    def _process_global_attention(
        ...
        Tuple[torch.Tensor, int, List[torch.Tensor], Tuple[torch.Tensor, torch.Tensor]],
    ]:
        """
        ...
        # by default, self.aa_block_size=1, which processes one block at a time
        for _ in range(self.aa_block_size):
            if not use_cache and self.use_causal_global:
                L = S * P
