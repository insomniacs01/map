...
class CameraHead(nn.Module):
    ...
    def trunk_fn(
        ...
        if use_cache and past_key_values_camera is None:
            past_key_values_camera = [None] * self.trunk_depth
        ...
        for _ in range(num_iterations):
            ...
            if self.causal_attention and not use_cache:
                L = S
                frame_ids = torch.arange(L, device=pose_tokens_modulated.device)
                future_frame = frame_ids.unsqueeze(1) < frame_ids.unsqueeze(0)
                attn_mask = future_frame.to(pose_tokens_modulated.dtype) * torch.finfo(
                    pose_tokens_modulated.dtype
                ).min
            else:
                attn_mask = None
                ...
                for idx in range(self.trunk_depth):
                    pose_tokens_modulated, block_kv = self.trunk[idx](
                        ...
                        past_key_values=(
                            ...
                            if past_key_values_camera is not None
                            else None
                        ),
                        use_cache=True,
                    )
