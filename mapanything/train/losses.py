        ...
        if is_multi_view:
            ...
            for i in range(n_views):
                ...
                if values_after_masking.numel() > 0:
                    view_loss_detail = float(values_after_masking.mean().detach())
            ...
            if values is not None:
                ...
                if values.numel() > 0:
                    loss_detail = float(values.mean().detach())
                    if loss_detail > 0:
...
def _finite_pixel_mask(tensor):
    if tensor is None:
        return None
    finite = torch.isfinite(tensor)
    if finite.ndim >= 4 and tensor.shape[-1] <= 4:
        finite = finite.all(dim=-1)
    return finite


def _safe_metric_scale(scale: torch.Tensor, min_scale: float = 1e-4, max_scale: float = 1.0e4):
    if scale is None:
        return None
...
def _sanitize_tensor(tensor):
    if tensor is None:
        return None
    return torch.nan_to_num(tensor, nan=0.0, posinf=1.0e4, neginf=-1.0e4)
...
def compute_normal_loss(points, gt_points, mask):
    ...
    """
    # Safety: avoid NaNs at invalid pixels leaking into the loss via 0*NaN.
    # We sanitize and zero-out invalid pixels before finite-difference operations.
    points = torch.nan_to_num(points, nan=0.0, posinf=0.0, neginf=0.0)
    gt_points = torch.nan_to_num(gt_points, nan=0.0, posinf=0.0, neginf=0.0)
    points = torch.where(mask[..., None], points, torch.zeros_like(points))
    gt_points = torch.where(mask[..., None], gt_points, torch.zeros_like(gt_points))
...
def compute_gradient_loss(prediction, gt_target, mask):
    ...
    # Avoid 0*NaN = NaN: explicitly zero-out invalid pixels.
...
class RelativePoseMetricLoss(MultiLoss):
    ...
    This loss is intended to stabilize metric translation learning for cooperative
    settings where large inter-agent baselines can otherwise be downweighted by
    geometry normalization schemes.
    """
    ...
    def _rotation_angle_deg_from_quats_xyzw(
        ...
        # Normalize + stabilize acos near dot==1.0.
        #
        # `acos` has an infinite slope at 1.0, and in practice we can hit dot==1
        # due to fp rounding / clamp interactions, which can yield inf/nan grads
        # and corrupt weights. We keep the *forward* value exact (angle==0 at dot==1)
        # but cap gradients for dot in [1-eps, 1] by using a detached residual.
        eps = 1e-4
    ...
    def compute_loss(self, batch, preds, **kw):
        ...
        q0 = torch.nan_to_num(
            preds[0]["cam_quats"].float(), nan=0.0, posinf=0.0, neginf=0.0
        )
        t0 = torch.nan_to_num(
            preds[0]["cam_trans"].float(), nan=0.0, posinf=0.0, neginf=0.0
        )
        ...
        trans_losses: list[torch.Tensor] = []
        rot_losses: list[torch.Tensor] = []
        cross_count = 0
        ...
        for view_idx in range(1, n_views):
            ...
            t_err = torch.linalg.norm((t_rel - gt_t_rel).float(), dim=-1)  # (B,)
            t_err = torch.nan_to_num(t_err, nan=1.0e4, posinf=1.0e4, neginf=1.0e4)
            if self.squared:
                t_err = t_err * t_err

            if only_cross_agent:
                agent_ids_i = self._get_agent_ids(
                    batch[view_idx].get("instance"), batch_size
                )
                mask_list = [
                ...
                if bool(mask.any()):
                    ...
                    if self.rot_weight > 0:
                        rot_err = self._rotation_angle_deg_from_quats_xyzw(
                            q_rel.float(), gt_q_rel.float()
                        )
                        rot_err = torch.nan_to_num(
                            rot_err, nan=1.0e4, posinf=1.0e4, neginf=1.0e4
                        )
        ...
        # Fallback: if only_cross_agent is requested but no samples were selected, supervise all.
        if only_cross_agent and not trans_losses:
            for view_idx in range(1, n_views):
                q_rel, t_rel = transform_pose_using_quats_and_trans_2_to_1(
                    q0, t0, preds[view_idx]["cam_quats"].float(), preds[view_idx]["cam_trans"].float()
                )
                gt_q_rel, gt_t_rel = transform_pose_using_quats_and_trans_2_to_1(
                    gt_q0,
                    gt_t0,
        ...
        trans_loss = (
            torch.stack(trans_losses, dim=0).mean()
            if trans_losses
            else torch.zeros((), device=device)
        )
        rot_loss = (
            torch.stack(rot_losses, dim=0).mean()
            if rot_losses
            else torch.zeros((), device=device)
        )
...
class ConfLoss(MultiLoss):
    ...
    def get_conf_log(self, x):
        # Clamp to avoid log(0) or log(negative) -> NaN/inf
        x_safe = torch.nan_to_num(x, nan=1.0, posinf=1.0e4, neginf=1.0e-6)
        x_safe = torch.clamp(x_safe, min=1e-6, max=1.0e4)
    ...
    def compute_loss(self, batch, preds, **kw):
        ...
        for loss_idx, (loss, msk, rep_type) in enumerate(selected_losses):
            ...
            # Drop non-finite loss entries to avoid NaN propagation.
            finite_loss = torch.isfinite(loss)
            if msk is not None:
                if finite_loss.ndim > msk.ndim:
                    finite_loss = finite_loss.all(dim=-1)
                msk = msk & finite_loss
            else:
                msk = finite_loss

            if msk is not None and not msk.any():
                continue
    ...
    def compute_loss(self, batch, preds, **kw):
        ...
        for loss_idx, (loss, msk, rep_type) in enumerate(selected_losses):
            ...
            if msk is None:
                ...
                loss_details[f"{self_name}_{rep_type}_loss_view{view_idx + 1}"] = float(
                    view_loss
                )
                avg_key = f"{self_name}_{rep_type}_loss_avg"
                if avg_key not in loss_details:
                    loss_details[avg_key] = float(view_loss)
                    running_avg_dict[f"{self_name}_{rep_type}_loss_valid_views"] = 1
                else:
                    valid_views = (
                        running_avg_dict[f"{self_name}_{rep_type}_loss_valid_views"] + 1
                    )
                    running_avg_dict[f"{self_name}_{rep_type}_loss_valid_views"] = (
                        valid_views
                    )
    ...
    def compute_loss(self, batch, preds, **kw):
        ...
        for loss_idx, (loss, msk, rep_type) in enumerate(exclude_selected_losses):
            ...
            if msk is None:
                ...
                avg_key = (
                    f"{self_name}_{rep_type}_bot{self.bottom_n_percent}%_loss_avg"
                )
                if avg_key not in loss_details:
                    loss_details[avg_key] = float(view_loss)
                    running_avg_dict[
    ...
    def get_all_info(self, batch, preds, dist_clip=None):
        ...
        for i in range(n_views):
            ...
            if "metric_scaling_factor" in preds[i].keys():
                ...
                scale = _safe_metric_scale(preds[i]["metric_scaling_factor"])
                curr_view_no_norm_pr_pts = preds[i]["pts3d"] / scale.unsqueeze(-1).unsqueeze(-1)
            else:
            ...
            gt_finite_mask = _merge_masks(
                _finite_pixel_mask(no_norm_gt_pts[-1]),
                _finite_pixel_mask(no_norm_gt_pts_cam[-1]),
                _finite_pixel_mask(no_norm_gt_depth[-1]),
                _finite_pixel_mask(gt_ray_directions[-1]),
            )
            pred_finite_mask = _merge_masks(
                _finite_pixel_mask(no_norm_pr_pts[-1]),
                _finite_pixel_mask(no_norm_pr_pts_cam[-1]),
                _finite_pixel_mask(no_norm_pr_depth[-1]),
                _finite_pixel_mask(pr_ray_directions[-1]),
            )
            finite_mask = _merge_masks(gt_finite_mask, pred_finite_mask)
            if finite_mask is not None:
                valid_masks[i] = valid_masks[i] & finite_mask
        ...
        valid_ratio = torch.stack(
            [mask.float().mean(dim=(1, 2)) for mask in valid_masks], dim=0
        ).mean(dim=0)
        valid_ratio_mask = valid_ratio >= self.min_scale_valid_ratio
        valid_metric_scale_mask = (
            metric_scale_mask & valid_gt_norm_factor_mask & valid_ratio_mask
        )
    ...
    def compute_loss(self, batch, preds, **kw):
        if self.scale_only:
            (
                ...
                pr_metric_norm_factor,
                valid_masks,
            ) = self._get_scale_only_info(batch, preds)
            n_views = len(batch)
            valid_ratio = torch.stack(
                [mask.float().mean(dim=(1, 2)) for mask in valid_masks], dim=0
            ).mean(dim=0)
            if gt_metric_norm_factor is not None and pr_metric_norm_factor is not None:
                if self.loss_in_log:
            ...
            details[f"{type(self).__name__}_scale_valid_ratio_avg"] = float(
                valid_ratio.mean().detach()
            )
...
class BEVCenternetDetLoss(MultiLoss):
    """
    Simple BEV CenterNet-style detection loss on top of a BEV heatmap + regression head.

    Expects:
      - preds[0]["bev_det"]["heatmap"]: (B, 1, H, W) after sigmoid
      - preds[0]["bev_det"]["reg"]: (B, 8, H, W)
      - batch[0]["vehicle_boxes"]: (B, M, 7) [x, y, z, l, w, h, yaw] in ego CARLA/UE frame
      - batch[0]["vehicle_boxes_mask"]: (B, M) bool/0-1
    """
    ...
    def __init__(
        ...
        *,
        ...
        max_gaussian_radius: int = 10,
        hard_neg_topk: int = 0,
        hard_neg_weight: float = 0.0,
        hard_neg_ignore_tgt: float = 0.1,
        score_clip: float = 1e-6,
    ) -> None:
        super().__init__()
        if len(x_range) != 2 or len(y_range) != 2:
            raise ValueError("x_range/y_range must be length-2 tuples.")
        ...
        self.max_gaussian_radius = int(max_gaussian_radius)
        self.hard_neg_topk = int(hard_neg_topk)
        self.hard_neg_weight = float(hard_neg_weight)
        self.hard_neg_ignore_tgt = float(hard_neg_ignore_tgt)
        self.score_clip = float(score_clip)

        if self.hard_neg_topk < 0:
            raise ValueError("hard_neg_topk must be >= 0")
        if self.hard_neg_weight < 0:
            raise ValueError("hard_neg_weight must be >= 0")
        if self.hard_neg_ignore_tgt < 0 or self.hard_neg_ignore_tgt >= 1:
            raise ValueError("hard_neg_ignore_tgt must be in [0, 1)")

    def get_name(self) -> str:
        return "BEVCenternetDetLoss"

    def _center_to_grid(self, centers_xy: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        gx = (centers_xy[..., 0] - self.x_min) / self.voxel_size
        gy = (centers_xy[..., 1] - self.y_min) / self.voxel_size
        return gx, gy

    def _focal_loss(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        pred = pred.float()
        target = target.float()
        pred = pred.clamp(self.score_clip, 1.0 - self.score_clip)
        pos = target.eq(1.0)
        neg = target.lt(1.0)
        neg_weights = torch.pow(1.0 - target, self.focal_beta)

        pos_loss = -torch.pow(1.0 - pred, self.focal_alpha) * torch.log(pred) * pos
        neg_loss = (
            -torch.pow(pred, self.focal_alpha)
            * torch.log(1.0 - pred)
            * neg_weights
            * neg
        )
    ...
    def _draw_gaussian(
        ...
        *,
        ...
        ix = int(math.floor(center_x))
        iy = int(math.floor(center_y))
        if ix < 0 or ix >= width or iy < 0 or iy >= height:
            return
        radius = max(self.min_gaussian_radius, min(int(radius), self.max_gaussian_radius))

        x0 = max(0, ix - radius)
        x1 = min(width - 1, ix + radius)
        y0 = max(0, iy - radius)
        y1 = min(height - 1, iy + radius)
        xs = torch.arange(x0, x1 + 1, device=heatmap.device, dtype=torch.float32)
        ys = torch.arange(y0, y1 + 1, device=heatmap.device, dtype=torch.float32)
        yy, xx = torch.meshgrid(ys, xs, indexing="ij")
        sigma = max(1.0, radius / 3.0)
        gauss = torch.exp(-((xx - center_x) ** 2 + (yy - center_y) ** 2) / (2 * sigma * sigma))
        patch = heatmap[batch_idx, 0, y0 : y1 + 1, x0 : x1 + 1]
        heatmap[batch_idx, 0, y0 : y1 + 1, x0 : x1 + 1] = torch.maximum(patch, gauss)

    def compute_loss(self, batch, preds, **kw):
        _ = kw
        if not preds or "bev_det" not in preds[0]:
            zero = torch.zeros((), device=preds[0]["pts3d"].device if preds else "cpu")
            return zero, {"BEVCenternetDetLoss_disabled": 1.0}

        det = preds[0]["bev_det"]
        heat_pred = det.get("heatmap")
        reg_pred = det.get("reg")
        if heat_pred is None or reg_pred is None:
            zero = torch.zeros((), device=preds[0]["pts3d"].device)
            return zero, {"BEVCenternetDetLoss_missing_pred": 1.0}

        if "vehicle_boxes" not in batch[0] or "vehicle_boxes_mask" not in batch[0]:
            zero = heat_pred.sum() * 0.0
            return zero, {"BEVCenternetDetLoss_missing_gt": 1.0}

        gt_boxes = batch[0]["vehicle_boxes"].to(heat_pred.device).float()
        gt_mask = batch[0]["vehicle_boxes_mask"].to(heat_pred.device)
        if gt_mask.dtype != torch.bool:
            gt_mask = gt_mask > 0

        batch_size, _, _ = gt_boxes.shape
        _, _, height, width = heat_pred.shape

        heat_tgt = torch.zeros_like(heat_pred)
        reg_targets = []
        reg_indices = []
        ...
        for b in range(batch_size):
            valid = gt_mask[b]
            if valid.sum() == 0:
                reg_targets.append(torch.empty((0, 8), device=heat_pred.device))
                reg_indices.append(torch.empty((0,), device=heat_pred.device, dtype=torch.long))
                continue

            boxes = gt_boxes[b][valid]
            centers = boxes[:, 0:2]
            gx, gy = self._center_to_grid(centers)
            ...
            for i in range(boxes.shape[0]):
                ...
                l = float(boxes[i, 3].item())
                w = float(boxes[i, 4].item())
                h = float(boxes[i, 5].item())
                radius = int(max(self.min_gaussian_radius, min(l, w) / (2.0 * self.voxel_size)))
                self._draw_gaussian(
                    heat_tgt,
                    batch_idx=b,
                    center_x=float(ix),
                ...
                offset_x = cx - ix
                offset_y = cy - iy
                z = float(boxes[i, 2].item())
                yaw = float(boxes[i, 6].item())
                tgt = torch.tensor(
                    [
                        offset_x,
                        offset_y,
                ...
                idx_list.append(iy * width + ix)
                tgt_list.append(tgt)

            if idx_list:
                reg_indices.append(torch.tensor(idx_list, device=heat_pred.device, dtype=torch.long))
                reg_targets.append(torch.stack(tgt_list, dim=0))
                total_boxes += len(idx_list)
            else:
                reg_targets.append(torch.empty((0, 8), device=heat_pred.device))
                reg_indices.append(torch.empty((0,), device=heat_pred.device, dtype=torch.long))

        heat_loss = self._focal_loss(heat_pred, heat_tgt) * self.heatmap_weight

        hard_neg_loss = torch.zeros((), device=heat_pred.device, dtype=torch.float32)
        if self.hard_neg_topk > 0 and self.hard_neg_weight > 0:
            # Penalize the highest-scoring negatives explicitly to reduce dense false positives.
            # We ignore cells inside the gaussian neighborhood (heat_tgt > hard_neg_ignore_tgt).
            batch_losses = []
            pred_map = heat_pred[:, 0]
            tgt_map = heat_tgt[:, 0]
            for b in range(int(batch_size)):
                neg_mask = tgt_map[b] <= float(self.hard_neg_ignore_tgt)
                neg_scores = pred_map[b][neg_mask]
                if neg_scores.numel() == 0:
                    continue
                k = min(int(self.hard_neg_topk), int(neg_scores.numel()))
                top_scores = torch.topk(neg_scores, k=k, largest=True, sorted=False).values
                top_scores = top_scores.clamp(self.score_clip, 1.0 - self.score_clip)
                batch_losses.append((-torch.log(1.0 - top_scores)).mean())
            if batch_losses:
                hard_neg_loss = torch.stack(batch_losses).mean() * self.hard_neg_weight

        reg_loss = torch.zeros((), device=heat_pred.device, dtype=torch.float32)
        if total_boxes > 0:
            for b in range(batch_size):
                if reg_indices[b].numel() == 0:
                    continue
                pred_flat = reg_pred[b].reshape(8, -1).transpose(0, 1)
                pred_sel = pred_flat.index_select(0, reg_indices[b])
                reg_loss = reg_loss + F.l1_loss(pred_sel, reg_targets[b], reduction="sum")
            reg_loss = reg_loss / float(total_boxes) * self.reg_weight

        total = heat_loss + reg_loss
        details = {
            "BEVCenternetDetLoss_heat": float(heat_loss.detach()),
            "BEVCenternetDetLoss_reg": float(reg_loss.detach()),
            "BEVCenternetDetLoss_num_boxes": float(total_boxes),
        }
        if self.hard_neg_topk > 0 and self.hard_neg_weight > 0:
            total = total + hard_neg_loss
            details["BEVCenternetDetLoss_hard_neg"] = float(hard_neg_loss.detach())
        return total, details


class VehicleBoxPointAlignmentLoss(MultiLoss):
    """
    Encourage vehicle geometry to stay correct during finetuning by supervising points inside GT 3D boxes.

    We identify "vehicle pixels" using the GT pointmap: a pixel is considered vehicle if its GT 3D point lies inside
    any GT vehicle box (in ego CARLA/UE frame). We then apply a point regression loss on those pixels only.

    This is useful for OPV2V because vehicles may be under-optimized unless explicitly monitored/weighted, and it gives
    a direct signal towards the qualitative goal: reconstructed vehicle points should sit inside the GT boxes.

    Expects:
      - batch[0]["vehicle_boxes"]: (B, M, 7) [x,y,z,l,w,h,yaw] in ego CARLA/UE
      - batch[0]["vehicle_boxes_mask"]: (B, M)
      - batch[v]["pts3d"]: (B, H, W, 3) GT points in OpenCV world frame
      - preds[v]["pts3d"]: (B, H, W, 3) Pred points in OpenCV world frame
    """
    ...
    def __init__(
        ...
        *,
        ...
        box_chunk_size: int = 16,
        view_indices: tuple[int, ...] | None = None,
        max_vehicle_points_per_sample: int = 20_000,
        loss_type: str = "smooth_l1",
        smooth_l1_beta: float = 1.0,
        detach_metric_scaling_factor: bool = False,
        compute_pred_in_box_ratio: bool = True,
        average_across_views: bool = True,
    ) -> None:
        super().__init__()
        self.box_margin = float(box_margin)
        self.box_chunk_size = int(box_chunk_size)
        self.view_indices = tuple(view_indices) if view_indices is not None else None
        self.max_vehicle_points_per_sample = int(max_vehicle_points_per_sample)
        self.loss_type = str(loss_type)
        self.smooth_l1_beta = float(smooth_l1_beta)
        self.detach_metric_scaling_factor = bool(detach_metric_scaling_factor)
        self.compute_pred_in_box_ratio = bool(compute_pred_in_box_ratio)
        self.average_across_views = bool(average_across_views)

        if self.box_chunk_size <= 0:
            raise ValueError("box_chunk_size must be positive.")
        if self.loss_type not in ("l1", "smooth_l1", "l2"):
    ...
    def get_name(self) -> str:
        return "VehicleBoxPointAlignmentLoss"

    def _opencv_to_carla(self, pts_cv: torch.Tensor) -> torch.Tensor:
        if pts_cv.numel() == 0:
            return pts_cv
        return torch.matmul(pts_cv, self._CARLA_TO_CV.to(pts_cv))

    def _points_in_boxes_any(
        self,
        points_carla_flat: torch.Tensor,
        boxes_carla: torch.Tensor,
        boxes_mask: torch.Tensor,
        *,
        margin: float,
    ) -> torch.Tensor:
        """Return (B, N) bool mask indicating whether each point is inside ANY box."""

        if points_carla_flat.ndim != 3 or points_carla_flat.shape[-1] != 3:
            raise ValueError("points_carla_flat must be (B, N, 3)")
        if boxes_carla.ndim != 3 or boxes_carla.shape[-1] != 7:
            raise ValueError("boxes_carla must be (B, M, 7)")

        batch_size, num_points, _ = points_carla_flat.shape
        device = points_carla_flat.device
        out = torch.zeros((batch_size, num_points), device=device, dtype=torch.bool)

        boxes_mask_bool = boxes_mask.to(device)
        if boxes_mask_bool.dtype != torch.bool:
            boxes_mask_bool = boxes_mask_bool > 0

        for b in range(batch_size):
            valid = boxes_mask_bool[b]
            if not bool(valid.any()):
                continue
            boxes = boxes_carla[b, valid].float()  # (M, 7)
            pts = points_carla_flat[b].float()  # (N, 3)

            m = boxes.shape[0]
            for start in range(0, m, self.box_chunk_size):
                chunk = boxes[start : start + self.box_chunk_size]  # (C, 7)
                cx = chunk[:, 0].unsqueeze(1)
                cy = chunk[:, 1].unsqueeze(1)
                cz = chunk[:, 2].unsqueeze(1)
                dx = (chunk[:, 3].abs() * 0.5 + margin).unsqueeze(1)
                dy = (chunk[:, 4].abs() * 0.5 + margin).unsqueeze(1)
                dz = (chunk[:, 5].abs() * 0.5 + margin).unsqueeze(1)
                yaw = chunk[:, 6].unsqueeze(1)
                c = torch.cos(-yaw)
                s = torch.sin(-yaw)

                px = pts[:, 0].unsqueeze(0) - cx
                py = pts[:, 1].unsqueeze(0) - cy
                pz = pts[:, 2].unsqueeze(0) - cz
                xloc = c * px - s * py
                yloc = s * px + c * py
                inside = (xloc.abs() <= dx) & (yloc.abs() <= dy) & (pz.abs() <= dz)
                out[b] |= inside.any(dim=0)

        return out

    def _reduce_point_loss(self, pred: torch.Tensor, gt: torch.Tensor) -> torch.Tensor:
        if self.loss_type == "l1":
            return (pred - gt).abs().sum(dim=-1)
        if self.loss_type == "l2":
            return ((pred - gt) ** 2).sum(dim=-1)
        # smooth_l1
        return F.smooth_l1_loss(pred, gt, reduction="none", beta=self.smooth_l1_beta).sum(dim=-1)

    def compute_loss(self, batch, preds, **kw):
        _ = kw
        if not preds or "pts3d" not in preds[0] or "pts3d" not in batch[0]:
            zero = torch.zeros((), device=preds[0]["pts3d"].device if preds else "cpu")
            return zero, {"VehicleBoxPointAlignmentLoss_disabled": 1.0}

        if "vehicle_boxes" not in batch[0] or "vehicle_boxes_mask" not in batch[0]:
            zero = preds[0]["pts3d"].sum() * 0.0
            return zero, {"VehicleBoxPointAlignmentLoss_missing_gt_boxes": 1.0}

        device = preds[0]["pts3d"].device
        boxes = batch[0]["vehicle_boxes"].to(device).float()
        boxes_mask = batch[0]["vehicle_boxes_mask"].to(device)

        view_ids = self.view_indices if self.view_indices is not None else tuple(range(len(batch)))
        total = preds[0]["pts3d"].sum() * 0.0
        details: dict[str, float] = {}
        views_used = 0
        total_vehicle_points = 0
        total_points = 0
        total_pred_in_points = 0

        for view_idx in view_ids:
            if view_idx < 0 or view_idx >= len(batch) or view_idx >= len(preds):
                continue
            if "pts3d" not in batch[view_idx] or "pts3d" not in preds[view_idx]:
                continue

            gt_pts_cv = batch[view_idx]["pts3d"].to(device).float()
            pred_pts_cv = preds[view_idx]["pts3d"].to(device).float()
            if self.detach_metric_scaling_factor and "metric_scaling_factor" in preds[view_idx]:
                # `pts3d` is produced as (base_pts3d * metric_scaling_factor). For this auxiliary
                # loss we want to avoid nudging the global metric scale (it can drift and hurt
                # detection), so we stop gradients into the scale factor while keeping the same
                # numeric points: base = pts3d / scale, pts3d_detached = base * scale.detach().
                scale = _safe_metric_scale(
                    preds[view_idx]["metric_scaling_factor"].to(device).float()
                )
                if scale.ndim == 1 and scale.shape[0] == pred_pts_cv.shape[0]:
                    scale_view = scale.view(-1, 1, 1, 1)
                    pred_pts_cv = pred_pts_cv / scale_view * scale_view.detach()
            if gt_pts_cv.shape != pred_pts_cv.shape:
                continue

            gt_pts_carla = self._opencv_to_carla(gt_pts_cv)
            pred_pts_carla = self._opencv_to_carla(pred_pts_cv)

            batch_size = gt_pts_carla.shape[0]
            pts_flat_gt = gt_pts_carla.reshape(batch_size, -1, 3)
            pts_flat_pred = pred_pts_carla.reshape(batch_size, -1, 3)

            vehicle_mask = self._points_in_boxes_any(
                pts_flat_gt, boxes, boxes_mask, margin=self.box_margin
            )
            if "valid_mask" in batch[view_idx]:
                valid = batch[view_idx]["valid_mask"].to(device)
                if valid.ndim == 3:
                    vehicle_mask &= valid.reshape(batch_size, -1) > 0

            num_vehicle = int(vehicle_mask.sum().detach().cpu())
            num_total = int(vehicle_mask.numel())
            details[f"VehicleBoxPointAlignmentLoss_vehicle_ratio_view{view_idx+1}"] = (
                float(num_vehicle) / float(max(1, num_total))
            )

            if num_vehicle == 0:
                continue

            # Optional subsampling (per-sample) to cap compute / stabilize gradients.
            if self.max_vehicle_points_per_sample > 0:
                keep = torch.zeros_like(vehicle_mask)
                for b in range(batch_size):
                    idx = torch.nonzero(vehicle_mask[b], as_tuple=False).flatten()
                    if idx.numel() == 0:
                        continue
                    if idx.numel() <= self.max_vehicle_points_per_sample:
                        keep[b, idx] = True
                        continue
                    perm = torch.randperm(idx.numel(), device=idx.device)[: self.max_vehicle_points_per_sample]
                    keep[b, idx[perm]] = True
                vehicle_mask = keep

            per_point = self._reduce_point_loss(pts_flat_pred, pts_flat_gt)
            loss_view = per_point[vehicle_mask].mean()
            total = total + loss_view
            views_used += 1
            details[f"VehicleBoxPointAlignmentLoss_loss_view{view_idx+1}"] = float(loss_view.detach())

            total_vehicle_points += int(vehicle_mask.sum().detach().cpu())
            total_points += int(vehicle_mask.numel())

            if self.compute_pred_in_box_ratio:
                pred_in = self._points_in_boxes_any(
                    pts_flat_pred, boxes, boxes_mask, margin=self.box_margin
                )
                denom = vehicle_mask.sum().clamp(min=1)
                numer = (pred_in & vehicle_mask).sum()
                ratio = (numer.float() / denom.float()).detach()
                details[f"VehicleBoxPointAlignmentLoss_pred_in_box_ratio_view{view_idx+1}"] = float(ratio)
                total_pred_in_points += int(numer.detach().cpu())

        if total_points > 0:
            details["VehicleBoxPointAlignmentLoss_vehicle_ratio"] = float(total_vehicle_points) / float(total_points)
        if self.compute_pred_in_box_ratio:
            details["VehicleBoxPointAlignmentLoss_pred_in_box_ratio"] = float(total_pred_in_points) / float(
                max(1, total_vehicle_points)
            )
        details["VehicleBoxPointAlignmentLoss_views_used"] = float(views_used)

        if self.average_across_views and views_used > 0:
            total = total / float(views_used)
...
class FactoredGeometryRegr3D(Criterion, MultiLoss):
    ...
    def get_all_info(self, batch, preds, dist_clip=None):
        ...
        for i in range(n_views):
            gt_info.append(
                {
                    "ray_directions": _sanitize_tensor(gt_ray_directions[i]),
                    self.depth_type_for_loss: _sanitize_tensor(gt_depth[i]),
                    "pose_trans": _sanitize_tensor(gt_pose_trans[i]),
                    "pose_quats": _sanitize_tensor(gt_pose_quats[i]),
                    "pts3d": _sanitize_tensor(gt_pts[i]),
                    "pts3d_cam": _sanitize_tensor(gt_pts_cam[i]),
                }
            )
            pred_info.append(
                {
                    "ray_directions": _sanitize_tensor(pr_ray_directions[i]),
                    self.depth_type_for_loss: _sanitize_tensor(pr_depth[i]),
                    "pose_trans": _sanitize_tensor(pr_pose_trans[i]),
                    "pose_quats": _sanitize_tensor(pr_pose_quats[i]),
                    "pts3d": _sanitize_tensor(pr_pts[i]),
                    "pts3d_cam": _sanitize_tensor(pr_pts_cam[i]),
                }
            )
...
class FactoredGeometryScaleRegr3D(Criterion, MultiLoss):
    ...
    def __init__(
        ...
        pose_trans_loss_weight=1,
        scale_loss_weight=1,
        min_scale_valid_ratio=0.0,
        compute_pairwise_relative_pose_loss=False,
        convert_predictions_to_view0_frame=False,
        compute_world_frame_points_loss=True,
        world_frame_points_loss_weight=1,
        direct_scale_loss=False,
        detach_metric_scaling_factor_from_geom: bool = False,
        scale_only=False,
    ):
        """
        ...
        Args:
            ...
            pose_trans_loss_weight (float): Weight to use for the pose trans loss. Default: 1.
            scale_loss_weight (float): Weight to use for the scale loss. Default: 1.
            min_scale_valid_ratio (float): Minimum valid depth ratio to enable scale supervision. Default: 0.0.
            compute_pairwise_relative_pose_loss (bool): If True, the pose loss is computed on the
                exhaustive pairwise relative poses. Default: False.
            convert_predictions_to_view0_frame (bool): If True, convert predictions to view0 frame.
                Use this if the predictions are not already in the view0 frame. Default: False.
            compute_world_frame_points_loss (bool): If True, compute the world frame pointmap loss. Default: True.
            world_frame_points_loss_weight (float): Weight to use for the world frame pointmap loss. Default: 1.
            direct_scale_loss (bool): If True, compute the scale loss directly on the
                predicted metric scaling factor instead of deriving it from predicted points.
            detach_metric_scaling_factor_from_geom (bool): If True, use `scale.detach()` in the
                denominator when converting metric predictions to an up-to-scale representation for
                geometry losses (points/depth/pose_trans branches). This blocks the `1/scale` gradient
                path. Note: if the model already multiplies geometry outputs by `metric_scaling_factor`
                upstream, leaving this False already cancels geometry->scale gradients, while setting
                it True can re-introduce geometry->scale gradients (treat as a gradient-routing knob).
            scale_only (bool): If True, compute only the scale loss and skip geometry losses.
        """
        super().__init__(criterion)
        self.norm_predictions = norm_predictions
        self.norm_mode = norm_mode
        ...
        self.pose_trans_loss_weight = pose_trans_loss_weight
        self.scale_loss_weight = scale_loss_weight
        self.min_scale_valid_ratio = min_scale_valid_ratio
        self.compute_pairwise_relative_pose_loss = compute_pairwise_relative_pose_loss
        self.convert_predictions_to_view0_frame = convert_predictions_to_view0_frame
        self.compute_world_frame_points_loss = compute_world_frame_points_loss
        self.world_frame_points_loss_weight = world_frame_points_loss_weight
        self.direct_scale_loss = direct_scale_loss
        self.detach_metric_scaling_factor_from_geom = bool(
            detach_metric_scaling_factor_from_geom
        )
        self.scale_only = scale_only

    def _get_scale_only_info(self, batch, preds):
        n_views = len(batch)
        in_camera0 = closed_form_pose_inverse(batch[0]["camera_pose"])
        no_norm_gt_pts = []
        valid_masks = []
        ...
        if valid_metric_scale_mask.any():
            ...
            for i in range(n_views):
                if "metric_scaling_factor" in preds[i]:
                    pred_scales.append(
                        _safe_metric_scale(preds[i]["metric_scaling_factor"])
                    )
            if pred_scales:
                pred_scale = torch.stack(pred_scales, dim=0).mean(dim=0)
                pred_scale = pred_scale.view(-1, 1, 1, 1)
                pr_metric_norm_factor = pred_scale[valid_metric_scale_mask]

        return gt_metric_norm_factor, pr_metric_norm_factor, valid_masks
    ...
    def get_all_info(self, batch, preds, dist_clip=None):
        ...
        for i in range(n_views):
            ...
            if "metric_scaling_factor" in preds[i].keys():
                scale = _safe_metric_scale(preds[i]["metric_scaling_factor"])
                # Divide by the predicted metric scaling factor to get the raw predicted points,
                # depth_along_ray, and pose_trans (up-to-scale representation).
                # Optionally detach scale from geometry losses to prevent metric-scale drift.
                scale_geom = (
                    scale.detach() if self.detach_metric_scaling_factor_from_geom else scale
                )
                curr_view_no_norm_pr_pts = pr_pts3d_in_view0 / scale_geom.unsqueeze(-1).unsqueeze(-1)
                curr_view_no_norm_pr_pts_cam = preds[i]["pts3d_cam"] / scale_geom.unsqueeze(-1).unsqueeze(-1)
                curr_view_no_norm_depth = curr_view_no_norm_depth / scale_geom.unsqueeze(-1).unsqueeze(-1)
                curr_view_no_norm_pr_pose_trans = pr_pose_trans_in_view0 / scale_geom
            else:
    ...
    def compute_loss(self, batch, preds, **kw):
        if getattr(self, "scale_only", False):
            (
                gt_metric_norm_factor,
                pr_metric_norm_factor,
                valid_masks,
...
class FactoredGeometryScaleRegr3DPlusNormalGMLoss(FactoredGeometryScaleRegr3D):
    ...
    def __init__(
        ...
        Args:
            ...
            direct_scale_loss (bool): If True, compute the scale loss directly on the
                predicted metric scaling factor instead of deriving it from predicted points.
            detach_metric_scaling_factor_from_geom (bool): If True, stop gradients from geometry
                losses into `metric_scaling_factor` (see base class for details).
            scale_only (bool): If True, compute only the scale loss and skip geometry losses.
        """
        super().__init__(
            criterion=criterion,
            ...
            min_scale_valid_ratio=min_scale_valid_ratio,
            compute_pairwise_relative_pose_loss=compute_pairwise_relative_pose_loss,
            convert_predictions_to_view0_frame=convert_predictions_to_view0_frame,
            compute_world_frame_points_loss=compute_world_frame_points_loss,
            world_frame_points_loss_weight=world_frame_points_loss_weight,
            direct_scale_loss=direct_scale_loss,
            detach_metric_scaling_factor_from_geom=detach_metric_scaling_factor_from_geom,
    ...
    def compute_loss(self, batch, preds, **kw):
        if getattr(self, "scale_only", False):
            return super().compute_loss(batch, preds, **kw)
        (
            gt_info,
            pred_info,
            valid_masks,
    ...
    def get_all_info(self, batch, preds, dist_clip=None):
        ...
        for i in range(n_views):
            ...
            if "metric_scaling_factor" in preds[i].keys():
                ...
                scale = _safe_metric_scale(preds[i]["metric_scaling_factor"])
                curr_view_no_norm_pr_pts = preds[i]["pts3d"] / scale.unsqueeze(-1).unsqueeze(-1)
                curr_view_no_norm_pr_pts_cam = preds[i]["pts3d_cam"] / scale.unsqueeze(-1).unsqueeze(-1)
                curr_view_no_norm_depth = curr_view_no_norm_depth / scale.unsqueeze(-1).unsqueeze(-1)
                curr_view_no_norm_pr_pose_trans = preds[i]["cam_trans"] / scale
            else:
                curr_view_no_norm_pr_pts = preds[i]["pts3d"]
                curr_view_no_norm_pr_pts_cam = preds[i]["pts3d_cam"]
                curr_view_no_norm_depth = curr_view_no_norm_depth
