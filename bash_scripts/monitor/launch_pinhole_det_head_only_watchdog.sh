...
RUN_TAG="${RUN_TAG:-$(date +%Y%m%d_%H%M%S)_pinhole_det_head_only_wd}"
...
NAS_RUN_DIR="${NAS_RUN_ROOT}/pinhole_det_head_only"

mkdir -p "${LOCAL_RUN_DIR}" "${NAS_RUN_DIR}"

LATEST_LINK="${REPO_ROOT}/experiments/long_runs/_latest_pinhole_det_head_only"
...
export CODEX_TASK_TAG="${CODEX_TASK_TAG:-pinhole_det_head_only_wd}"
...
if [ -z "${GEOM_CKPT:-}" ] && [ -f "${DEFAULT_GEOM}" ]; then
  GEOM_CKPT="${DEFAULT_GEOM}"
fi
...
ENTRY_SCRIPT="${REPO_ROOT}/bash_scripts/train/run_pinhole_det_head_only_local_sync.sh"
