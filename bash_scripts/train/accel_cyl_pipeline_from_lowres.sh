...
ROOT="${1:?Usage: $0 <run_root> <cyl_lowres_ckpt>}"
LOWRES_CKPT="${2:?Usage: $0 <run_root> <cyl_lowres_ckpt>}"
...
run_phase() {
  ...
  mkdir -p "${out_dir}"
  CUDA_VISIBLE_DEVICES=4,5,6,7 PYTHONPATH=$(pwd) "${TORCHRUN}" --nproc_per_node=4 --master_port=29631 \
...
echo "Cyl pipeline finished. Logs under ${ROOT}."
