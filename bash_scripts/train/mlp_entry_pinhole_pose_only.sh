...
if [ "${missing}" -ne 0 ]; then
  ...
  export MLP_WORKER_0_PORT="${MLP_WORKER_0_PORT:-29640}"
...
NAS_RUN_ROOT="${NAS_RUN_ROOT:-${REPO_ROOT}/experiments/long_runs/$(date +%Y%m%d_%H%M%S)_pinhole_pose_only_a800_8g}"
LOCAL_ROOT="${LOCAL_ROOT:-/tmp/mapanything_runs/pinhole_pose_only_a800_8g}"
LOCAL_RUN_DIR="${LOCAL_ROOT}/pinhole_pose_only"
NAS_RUN_DIR="${NAS_RUN_ROOT}/pinhole_pose_only"
...
LR="${LR:-5e-5}"
MIN_LR="${MIN_LR:-5e-6}"
PRETRAINED_CKPT="${PRETRAINED_CKPT:-${REPO_ROOT}/experiments/long_runs/20260129_pinhole_pose_strong_a800_10g/pinhole_pose_metric_strong/checkpoint-best.pth}"
