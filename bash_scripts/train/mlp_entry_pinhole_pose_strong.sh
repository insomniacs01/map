...
if [ "${missing}" -ne 0 ]; then
  ...
  export MLP_WORKER_GPU="${MLP_WORKER_GPU:-10}"
...
NAS_RUN_ROOT="${NAS_RUN_ROOT:-${REPO_ROOT}/experiments/long_runs/$(date +%Y%m%d_%H%M%S)_pinhole_pose_metric_strong_a800_10g}"
LOCAL_ROOT="${LOCAL_ROOT:-/tmp/mapanything_runs/pinhole_pose_metric_strong_a800_10g}"
LOCAL_RUN_DIR="${LOCAL_ROOT}/pinhole_pose_metric_strong"
NAS_RUN_DIR="${NAS_RUN_ROOT}/pinhole_pose_metric_strong"
...
EPOCHS="${EPOCHS:-20}"
PRETRAINED_CKPT="${PRETRAINED_CKPT:-${REPO_ROOT}/checkpoints/facebook_map-anything-v1.pth}"
