...
def _camera_index_from_key(cam_key: str) -> int:
    match = re.search(r"(\d+)$", str(cam_key))
    if match is None:
        raise ValueError(f"Unable to parse camera index from key: {cam_key}")
...
def build_single_raw(images_root: Path, depth_root: Path, split: str, info: FrameInfo) -> Tuple[List[Dict], List[Dict]]:
    ...
    for cam_key in sorted(cam_poses_ego.keys()):
        ...
        camera_index = _camera_index_from_key(cam_key)
        raw_views.append(
            {
                "img": img,
                "depth_z": depth,
                ...
                "agent_index": torch.tensor([0], dtype=torch.long),
                "camera_index": torch.tensor([camera_index], dtype=torch.long),
            }
        )
        camera_infos.append(
            {
...
def build_coop_raw(images_root: Path, depth_root: Path, split: str, info: FrameInfo) -> Tuple[List[Dict], List[Dict]]:
    ...
    agent_to_index = {agent_id: idx for idx, agent_id in enumerate(info.coop_agents)}
    raw_views: List[Dict] = []
    camera_infos: List[Dict] = []
    for agent in info.coop_agents:
        meta = metadata[agent]
            ...
            raw_views.append(
                {
                    ...
                    "agent_index": torch.tensor([int(agent_to_index[agent])], dtype=torch.long),
                    "camera_index": torch.tensor([camera_index], dtype=torch.long),
                }
            )
            camera_infos.append(
